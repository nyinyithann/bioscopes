  ...
  let make = (~title: option<string>, ~poster_path: option<string>) => {
    ...
    <button
      ...
      {Js.String2.length(imgLink) > 0
        ? <img
            alt="A poster" src={imgLink} className="w-[13rem] h-[18rem] flex-shrink-0 transform duration-300 group-hover:saturate-150"
          />
        : <div> {"placeholder here"->string} </div>}
      <p className="break-words text-[0.9rem] transform duration-300 group-hover:scale-110 group-hover:p-2  group-hover:text-yellow-200
