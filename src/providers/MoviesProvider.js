...
function filterTVShows(results, apiParams) {
  if (apiParams.TAG === /* Search */2) {
    return Belt_Array.keep(results, (function (x) {
                  return Util.getOrEmptyString(x.media_type).toLowerCase() !== "tv";
                }));
  } else {
    return results;
  }
...
function reducer(state, action) {
  ...
  switch (action.TAG | 0) {
    ...
    case /* SuccessMovies */2 :
        ...
        var apiParams = action._0;
        return {
                apiParams: apiParams,
                movies: {
                  dates: movies.dates,
                  page: movies.page,
                  results: MovieModel.unique(Js_option.getWithDefault([], state.movies.results), filterTVShows(Js_option.getWithDefault([], movies.results), apiParams)),
